...
# Gen I pokered.sym only defines wPartyMon1..6; corrupt RAM or ROM hacks can set wPartyCount > 6.
_MAX_PARTY_SIZE_RED = 6
...
def _project_root() -> Path:
    """Directory that contains `pokemonred_puffer/` (repository root when installed editable)."""
    return Path(__file__).resolve().parent.parent


def resolve_state_dir(state_dir: str | Path) -> Path:
    """Resolve `env.state_dir`: relative paths are from project root (not process CWD)."""
    p = Path(state_dir)
    if p.is_absolute():
        return p.resolve()
...
def _resolve_state_fallback_from_sav(sav_path: Path) -> Path | None:
    same_name_state = sav_path.with_suffix(".state")
    if same_name_state.is_file():
        return same_name_state.resolve()
    candidates = sorted(sav_path.parent.glob("*.state"))
    if not candidates:
        return None
...
class RedGymEnv(Env):
    try:
        ...
        # Fallback for restricted environments where POSIX semaphores are forbidden.
        import threading

        lock = threading.Lock()
    try:
        # Used to keep unique env ids across workers.
        # NOTE: In some sandboxed environments, creating POSIX shared memory is forbidden.
    ...
    def __init__(self, env_config: DictConfig):
        ...
        init_state_path_override = env_config.get("init_state_path")
        if init_state_path_override:
            self.init_state_path = Path(init_state_path_override).expanduser().resolve()
            self.state_dir = self.init_state_path.parent
        else:
            self.state_dir = resolve_state_dir(env_config.state_dir)
            self.init_state_path = self.state_dir / f"{self.init_state_name}.state"
        if not self.init_state_path.exists():
            raise FileNotFoundError(
                f"PyBoy 시작 파일이 없습니다 (.state 또는 .sav): {self.init_state_path} "
                f"(env.init_state={self.init_state_name!r}, env.state_dir={env_config.get('state_dir')!r}, "
                f"env.init_state_path={init_state_path_override!r})"
            )
        self._init_from_sav = self.init_state_path.suffix.lower() == ".sav"
        self._sav_template_state: Optional[bytes] = None
        ...
        # Bit 0 of wObtainedBadges = Boulder Badge (Pewter / first gym leader).
        self.end_episode_on_first_gym = env_config.get("end_episode_on_first_gym", True)
        ...
        from pokemonred_puffer.save_town_state import quarantine_invalid_sidecar_ram

        quarantine_invalid_sidecar_ram(Path(env_config.gb_path))
        if self._init_from_sav:
            from pokemonred_puffer.save_town_state import sav_path_to_ram_bytes

            _ram = io.BytesIO(sav_path_to_ram_bytes(self.init_state_path))
            try:
                self.pyboy = PyBoy(str(env_config.gb_path), ram_file=_ram, **pyboy_kw)
            except (KeyError, TypeError) as exc:
                fallback_state = _resolve_state_fallback_from_sav(self.init_state_path)
                if fallback_state is None:
                    raise RuntimeError(
                        "This PyBoy build does not support `ram_file`, and no fallback `.state` "
                        f"exists next to {self.init_state_path}."
                    ) from exc
                self.init_state_path = fallback_state
                self.init_state_name = fallback_state.stem
                self._init_from_sav = False
                self.pyboy = PyBoy(str(env_config.gb_path), **pyboy_kw)
            else:
                _warmup = int(env_config.get("init_sav_warmup_ticks", 0))
                for _ in range(_warmup):
                    self.pyboy.tick(render=False)
                _snap = io.BytesIO()
                self.pyboy.save_state(_snap)
                _snap.seek(0)
                self._sav_template_state = _snap.read()
        else:
            self.pyboy = PyBoy(str(env_config.gb_path), **pyboy_kw)
        ...
        with RedGymEnv.lock:
            if RedGymEnv.env_id is not None:
                env_id = (
                ...
                # Fallback for restricted environments (no POSIX shm).
                RedGymEnv._env_id_counter += 1
                self.env_id = RedGymEnv._env_id_counter
    ...
    def reset(self, seed: Optional[int] = None, options: Optional[dict[str, Any]] = None):
        ...
        # Migration / swarm: load injected RAM; otherwise always reload init .state so every
        # episode starts at env.init_state (e.g. PewterCity), not wherever the last step ended.
        if options.get("state", None) is not None:
            self.pyboy.load_state(io.BytesIO(options["state"]))
        else:
            if self._sav_template_state is not None:
                self.pyboy.load_state(io.BytesIO(self._sav_template_state))
            else:
            ...
            if self.save_state and self.first:
                state = io.BytesIO()
        ...
        self._prev_boulder_badge = bool(self.read_m("wObtainedBadges") & 1)

        return self._get_obs(), infos

    def seed(self, seed: Optional[int] = None):
        """PufferLib ``GymnasiumPufferEnv.seed`` calls this when ``reset(seed=...)`` fails."""
    ...
    def _masked_battle_type(self) -> np.ndarray:
        ...
        try:
            enemy_species = int(self.read_m("wEnemyMonSpecies"))
        except Exception:
            return np.array([0], dtype=np.uint8)
    ...
    def _get_obs(self):
        ...
        return (
            ...
            | {
                ...
                "battle_type": self._masked_battle_type(),
    ...
    def set_perfect_iv_dvs(self):
        party_size = self._party_count_clamped()
    ...
    def is_a_press_action(self, action: int) -> bool:
        return VALID_ACTIONS[int(action)] == WindowEvent.PRESS_BUTTON_A
    ...
    def step(self, action):
        ...
        if self.end_episode_on_first_gym:
            has_boulder = bool(self.read_m("wObtainedBadges") & 1)
            if has_boulder and not self._prev_boulder_badge:
                reset = True
                info["first_gym_cleared"] = True
            self._prev_boulder_badge = has_boulder
    ...
    def run_action_on_emulator(self, action):
        ...
        # One last tick just in case (render=True: policy obs reads screen buffer; must stay True)
    ...
    def agent_stats(self, action):
        ...
        return {
            ...
            "stats": {
                ...
                "event": float(self.progress_reward["event"]),
            ...
            # Remove padding
            "pokemon_exploration_map": self.build_pokemon_exploration_rgb(),
    ...
    def get_explore_map(self):
        ...
        for _tileset, coord_dict in self.seen_coords.items():
            for (x_pos, y_pos, map_n), v in coord_dict.items():
                gy, gx = local_to_global(y_pos, x_pos, map_n)
                if not (0 <= gy < explore_map.shape[0] and 0 <= gx < explore_map.shape[1]):
                    print(
                        f"coord out of bounds! global: ({gx}, {gy}) game: ({x_pos}, {y_pos}, {map_n})"
                    )
                else:
                    explore_map[gy, gx] = v
    ...
    def _party_count_clamped(self) -> int:
        """Use for loops over wPartyMon{{1..N}} symbols; N is capped at 6 in Red/Blue."""
        return max(0, min(int(self.read_m("wPartyCount")), _MAX_PARTY_SIZE_RED))

    def read_short(self, addr: str | int) -> int:
        if isinstance(addr, str):
            _, addr = self.pyboy.symbol_lookup(addr)
        data = self.pyboy.memory[addr : addr + 2]
        # Game Boy WRAM: 16-bit values are little-endian (low byte first).
    ...
    def read_party(self):
        ...
        party_length = max(0, min(int(self.read_m("wPartyCount")), _MAX_PARTY_SIZE_RED))
        if party_length <= 0:
    ...
    def update_max_op_level(self):
        # opp_base_level = 5
        enemy_party_count = max(0, min(int(self.read_m("wEnemyPartyCount")), _MAX_PARTY_SIZE_RED))
        opponent_level = max(
            [0]
    ...
    def update_tm_hm_obtained_move_ids(self):
        # TODO: Make a hook
        # Scan party
        for i in range(self._party_count_clamped()):
            _, addr = self.pyboy.symbol_lookup(f"wPartyMon{i+1}Moves")
            for move_id in self.pyboy.memory[addr : addr + 4]:
                move_id_int = int(move_id)
                if 0 < move_id_int < len(self.obtained_move_ids):
                    self.obtained_move_ids[move_id_int] = 1
        """
    ...
    def reverse_damage(self):
        # WRAM 16-bit values are little-endian (low byte at addr, high at addr+1).
        fixed = 128
        lo = fixed & 0xFF
        hi = (fixed >> 8) & 0xFF
        for i in range(self._party_count_clamped()):
            ...
            self.pyboy.memory[wPartyMonHP] = lo
            self.pyboy.memory[wPartyMonHP + 1] = hi
            self.pyboy.memory[wPartymonMaxHP] = lo
            self.pyboy.memory[wPartymonMaxHP + 1] = hi
    ...
    def get_items_in_bag(self) -> Iterable[Items]:
        num_bag_items = int(self.read_m("wNumBagItems"))
        num_bag_items = max(0, min(num_bag_items, MAX_ITEM_CAPACITY))
        _, addr = self.pyboy.symbol_lookup("wBagItems")
        start_addr = int(addr)
        stop_addr = start_addr + 2 * num_bag_items
        if num_bag_items == 0 or stop_addr <= start_addr:
            return []
        return [Items(i) for i in self.pyboy.memory[start_addr:stop_addr][::2]]
    ...
    def get_required_items(self) -> set[str]:
        wNumBagItems = int(self.read_m("wNumBagItems"))
        wNumBagItems = max(0, min(wNumBagItems, MAX_ITEM_CAPACITY))
        _, wBagItems = self.pyboy.symbol_lookup("wBagItems")
        start_addr = int(wBagItems)
        stop_addr = start_addr + 2 * wNumBagItems
        if wNumBagItems == 0 or stop_addr <= start_addr:
            return set()
        bag_items = self.pyboy.memory[start_addr:stop_addr:2]
    ...
    def check_num_bag_items(self):
        _, wNumBagItems = self.pyboy.symbol_lookup("wNumBagItems")
        numBagItems = self.read_m(wNumBagItems)
        if numBagItems >= MAX_ITEM_CAPACITY and not getattr(self, "_warned_full_bag_once", False):
            self._warned_full_bag_once = True
            _, wBagItems = self.pyboy.symbol_lookup("wBagItems")
            bag = self.pyboy.memory[wBagItems : wBagItems + 40]
            raw_ids = [int(bag[2 * i]) for i in range(min(numBagItems, MAX_ITEM_CAPACITY))]
            labels = [Items(i).name for i in raw_ids]
            print(
                f"WARNING: env id {int(self.env_id)} bag at capacity ({numBagItems} items): {labels}"
            )
