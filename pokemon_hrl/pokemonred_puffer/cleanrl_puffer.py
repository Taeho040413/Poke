...
def rollout(
    ...
    mp = pathlib.Path(model_path).expanduser().resolve()
    print(f"[evaluate] 모델 로드 중: {mp}", flush=True)
    agent = torch.load(mp, map_location=device, weights_only=False)
    print(f"[evaluate] 모델 로드 완료: {mp}", flush=True)
...
def wandb_environment_scalar_allowed(key: str, mode: str) -> bool:
    """Filter wandb environment/* scalars. reward_only keeps reward shaping only."""
    if mode == "full":
        return True
    # Flat keys (legacy)
    if key in ("reward_sum", "stats/reward_sum"):
        return True
    if key.startswith("reward/"):
        return True
    # RedGymEnv.agent_stats nests rewards under stats/reward/* (pufferlib unroll_nested_dict).
    if key.startswith("stats/reward/"):
        return True
    return False
...
class CleanPuffeRL:
    ...
    # train.py에서 체크포인트 로드 후 __post_init__ 맨 끝에서 출력 (대시보드 0;0H보다 늦게)
    ...
    def __post_init__(self):
        ...
        if self.resume_trainer_state is not None and getattr(
            self.config, "load_optimizer_state", False
        ):
            self.optimizer.load_state_dict(self.resume_trainer_state["optimizer_state_dict"])
            self.global_step = int(self.resume_trainer_state.get("global_step", 0))
            self.epoch = int(self.resume_trainer_state.get("update", 0))

        if self.resume_trainer_state is not None:
            self._restore_lstm_state(self.resume_trainer_state)

        self.last_log_time = time.time()
        self.early_stop_check_print_interval = 120
        self._last_early_stop_check_print = -120
        ...
        self.log = False
        self._first_gym_policy_saved = False
        ...
        if self.resume_load_log_lines:
            for line in self.resume_load_log_lines:
    ...
    def evaluate(self):
        ...
        while not self.experience.full:
            with self.profile.env:
                ...
                env_id = np.asarray(env_id).tolist()
            ...
            with self.profile.eval_misc:
                ...
                for i in info:
                    if (
                        getattr(self.config, "save_on_first_gym", True)
                        and not self._first_gym_policy_saved
                        and isinstance(i, dict)
                        and i.get("first_gym_cleared")
                    ):
                        self._first_gym_policy_saved = True
                        p = self.save_first_gym_checkpoint()
                        if self.config.verbose:
                            print(f"First gym cleared — policy saved to {p}")
        ...
        with self.profile.eval_misc:
            ...
            if self.states and self.config.early_stop:
                to_delete = []
                should_print_check = (
                    self.profile.uptime - self._last_early_stop_check_print
                    >= self.early_stop_check_print_interval
                )
                ...
                if should_print_check:
                    self._last_early_stop_check_print = self.profile.uptime
            ...
            _wandb_env_mode = getattr(self.config, "wandb_environment_metrics", "reward_only")
            ...
            if (
                _wandb_env_mode == "full"
                and all(k in self.infos.keys() for k in ["env_ids", "species", "levels", "moves"])
    ...
    def train(self):
        ...
        with self.profile.train_misc:
            ...
            # wandb must not be gated on profile.update(); otherwise only W&B "System" metrics appear
            # (no Overview/losses/performance) whenever the SPS tick returns False.
            if self.global_step > 0 and time.time() - self.last_log_time > 5.0:
                self.last_log_time = time.time()

                if self.wandb_client is not None:
                    self.wandb_client.log(
                        {
                            "Overview/SPS": float(self.profile.SPS),
                            "Overview/agent_steps": int(self.global_step),
                            "Overview/learning_rate": float(
    ...
    def _upload_wandb_checkpoint(self, model_path: str) -> None:
    ...
    def close(self, *, skip_checkpoint: bool = False):
        self.vecenv.close()
        if self.config.verbose:
            self.utilization.stop()

        if skip_checkpoint or not getattr(self.config, "save_on_exit", False):
            return

        model_path = self.save_checkpoint()
        self._upload_wandb_checkpoint(model_path)

    def _print_checkpoint_log(
        self,
        model_path: str,
        state_path: str,
        *,
        ...
        print(f"[checkpoint] 모델 경로: {model_path}")
        print(f"[checkpoint] 트레이너 상태 경로: {state_path}")
        if game_path:
            print(f"[checkpoint] 게임 세이브 경로: {game_path}")

    def _restore_lstm_state(self, trainer_state: dict) -> None:
        if self.experience.lstm_h is None or self.experience.lstm_c is None:
            return
        saved_h = trainer_state.get("lstm_h")
        saved_c = trainer_state.get("lstm_c")
        if saved_h is None or saved_c is None:
            return
        saved_h = saved_h.to(self.config.device)
        saved_c = saved_c.to(self.config.device)
        if (
            saved_h.shape != self.experience.lstm_h.shape
            or saved_c.shape != self.experience.lstm_c.shape
        ):
            print(
                "[resume] LSTM hidden state shape mismatch — skipping restore "
                f"(saved {tuple(saved_h.shape)} vs current {tuple(self.experience.lstm_h.shape)})",
                flush=True,
            )
            return
        self.experience.lstm_h.copy_(saved_h)
        self.experience.lstm_c.copy_(saved_c)
        print("[resume] LSTM hidden state 복원 완료", flush=True)

    def _env_with_pyboy(self):
    ...
    def _save_game_checkpoint(self, path: str) -> str | None:
        if not getattr(self.config, "save_game_on_checkpoint", False):
            return None
        base = self._env_with_pyboy()
        if base is None:
            return None
        import io

        buffer = io.BytesIO()
        base.pyboy.save_state(buffer)
        payload = buffer.getvalue()
        latest_path = os.path.join(path, "game_latest.state")
        tmp = latest_path + ".tmp"
        with open(tmp, "wb") as handle:
            handle.write(payload)
        os.replace(tmp, latest_path)
        numbered_path = os.path.join(path, f"game_{self.epoch:06d}.state")
        tmp_numbered = numbered_path + ".tmp"
        with open(tmp_numbered, "wb") as handle:
            handle.write(payload)
        os.replace(tmp_numbered, numbered_path)
        return latest_path

    def _checkpoint_run_dir(self) -> str:
        config = self.config
        path = os.path.join(config.data_dir, config.exp_id)
        os.makedirs(path, exist_ok=True)
        return path

    def _write_trainer_state(self, path: str, *, model_name: str) -> str:
        ...
        if self.experience.lstm_h is not None and self.experience.lstm_c is not None:
            state["lstm_h"] = self.experience.lstm_h.detach().cpu()
            state["lstm_c"] = self.experience.lstm_c.detach().cpu()
        tmp = state_path + ".tmp"
        torch.save(state, tmp)
        os.replace(tmp, state_path)
        return state_path

    def _save_model_latest(self, path: str) -> str:
    ...
    def save_checkpoint(self, *, force: bool = False):
        ...
        latest_path = self._save_model_latest(path)

        game_path = self._save_game_checkpoint(path)

        if os.path.exists(model_path) and not force:
            state_path = self._write_trainer_state(path, model_name="model_latest.pt")
            self._print_checkpoint_log(latest_path, state_path, game_path=game_path)
            return latest_path

        tmp_model = model_path + ".tmp"
        torch.save(self.uncompiled_policy, tmp_model)
        os.replace(tmp_model, model_path)
        state_path = self._write_trainer_state(path, model_name=model_name)
        self._print_checkpoint_log(model_path, state_path, game_path=game_path)
        return model_path

    def save_first_gym_checkpoint(self):
        """Save policy weights to model_first_gym.pt under the run directory."""
        config = self.config
        path = os.path.join(config.data_dir, config.exp_id)
        if not os.path.exists(path):
            os.makedirs(path)
        model_path = os.path.join(path, "model_first_gym.pt")
        torch.save(self.uncompiled_policy, model_path)
        return model_path

    def done_training(self):
        one_epoch_targets = set(self.config.one_epoch or [])
        if one_epoch_targets and self.states:
            # state keys are tuples of milestone names; stop once every configured
            # one_epoch target appears in at least one discovered state key.
            seen_targets = {
                target
                for key in self.states.keys()
                if isinstance(key, tuple)
                for target in one_epoch_targets
                if target in key
            }
            one_epoch_done = seen_targets == one_epoch_targets
        else:
    ...
    def __exit__(self, exc_type, *_args):
        save_on_exit = bool(getattr(self.config, "save_on_exit", False))
        save_policy_on_interrupt = bool(
            getattr(self.config, "save_policy_on_interrupt", True)
        )

        if exc_type is KeyboardInterrupt:
            if save_policy_on_interrupt:
                policy_path = self.save_policy_only()
                print(
                    "[checkpoint] Ctrl+C — 정책만 저장 (자동 재개용 아님): "
                    f"{policy_path}",
                    flush=True,
                )
            else:
                print("[checkpoint] Ctrl+C — 체크포인트 저장 없음", flush=True)
            self.close(skip_checkpoint=True)
            return False

        if exc_type is None:
            if save_on_exit:
                print("Done training.")
                model_path = self.save_checkpoint(force=True)
                self._upload_wandb_checkpoint(model_path)
            else:
                print(
                    "[checkpoint] 학습 종료 — 재개 체크포인트 미갱신 "
                    "(train.save_on_exit=false, goal/interval 세이브만 재개에 사용)",
                    flush=True,
                )
        else:
            print(f"Training stopped: {exc_type.__name__}", flush=True)
...
def print_dashboard(
    ...
    left.add_column(f"{c1}User Stats", justify="left", width=30)
    left.add_column(f"{c1}Value", justify="right", width=12)
    right.add_column(f"{c1}User Stats", justify="left", width=30)
    right.add_column(f"{c1}Value", justify="right", width=12)
    i = 0
